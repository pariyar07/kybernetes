...
| `permission_boundary` | The loop needs to know whether an action is allowed, denied, requires approval, crosses external/irreversible/private surfaces, or releases information outside the current boundary. For release, name the destination, purpose, minimum payload scope, exclusions, inspection or sanitization, approval evidence, and residual risk. | The action is local-only work within a clear, reversible, low-risk boundary. Permission checks should not become a ritual for tiny edits. | Bind to runtime permissions, sandbox, tool approval, or policy surfaces; otherwise write the assumption and ask before crossing the boundary. Tool permission does not authorize an unbounded payload release. If scope or authority is unclear, reduce and inspect the payload or use a local/no-send fallback. |
...
| `out_of_band_steering` | A session can be steered or answered by more than one attached client (a second device pairs with or syncs to the same running loop), and pairing/attach state plus answer provenance can be recorded. | The session has a single implicit client. Do not assume a second device is attached or watching unless the active binding confirms pairing. | Bind to the runtime's multi-device/remote-steering surface when available; record which device answered and its context freshness. Prefer requiring the most-current-context client to answer irreversible or production-impacting prompts rather than accepting the first response from any attached device. |
