...
| `permission_boundary` | Respect permission modes, allowed/denied/ask rules, directory access, managed settings, hooks, and tool approval prompts. A built-in guardrail now blocks modification of session transcript files; do not attempt to route around it. | Autonomy grants do not imply permission for irreversible, external, secret-bearing, customer-visible, production, billing, publishing, or destructive actions. | Record allowed/denied/ask status and exact unlocked action in `control.md`. |
...
`/doctor` may suggest trimming `CLAUDE.md` content Claude could otherwise
derive from the codebase itself. Treat that as project-instruction hygiene, not
as guidance to trim `control.md` or `verification.md`; the trust pair stays
canonical regardless of `CLAUDE.md` size.
