...
| `external_tool_provider` | Use MCP, browser, docs, GitHub, web, filesystem, shell, plugins, or app tools as bounded sensors/actuators. MCP interactive auth (login redirect) is a stable feature, not experimental; remote plugins are enabled by default with a broader catalog. | Tool availability and side effects are governed by the active sandbox and approval policy. | Record side effects, permission decision, result pointer, and whether output is sensor evidence. |
...
| `permission_boundary` | Respect approval policy, sandbox mode, `/permissions`, MCP/app side-effect approvals, workspace roots, and network policy. Approval policy includes an app-approval mode where declared read-only actions run automatically while writes still prompt; treat this as a distinct posture from `never`, not a relaxation of the write boundary. | Autonomy grants do not imply permission for irreversible, external, secret-bearing, customer-visible, production, billing, publishing, or destructive actions. | Record allowed/denied/ask status and exact unlocked action in `control.md`. |
...
Codex may warn when high subagent concurrency is combined with ultra reasoning
effort. Treat that warning as a signal to lower concurrency or reasoning
effort, not as permission to ignore `[agents] max_threads`/`max_depth`.
