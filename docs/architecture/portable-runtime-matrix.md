...
| --- | --- | --- | --- | --- |
| `durable_objective` | `/goal` on the CLI, confirmed with `edit`/`pause`/`resume`/`clear` subverbs; non-CLI surfaces unconfirmed to expose the same surface. Advisory. | Lead prompt plus control record; no universal persistent command assumed. | Objective, DONE, verifier, and constraints in prompt or `control.md`. | Mirror recovery pointer and objective state into `control.md`. |
